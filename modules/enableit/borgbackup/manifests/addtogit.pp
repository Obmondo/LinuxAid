...
define borgbackup::addtogit (
  ...
  if $passphrase == 'random' {
    ...
    exec { "create passphrase file ${title}":
      command => "cat /dev/random |tr -dc _A-Z-a-z-0-9 | head -c30 | gpg --encrypt --always-trust ${keys} > ${git_home}/${::fqdn}/${reponame}_pass.gpg", #lint:ignore:140chars
  ...
  exec { "reencrypt passphrase file ${title}":
    command => "gpg --decrypt ${git_home}/${::fqdn}/${reponame}_pass.gpg | gpg --encrypt --always-trust ${keys} > ${git_home}/${::fqdn}/${reponame}_pass.gpg", #lint:ignore:140chars
    require => Exec["create passphrase file ${title}"],
    unless  => [
      # check if file is encrypted with correct keys
      "gpg --decrypt -v --output /dev/null ${git_home}/${::fqdn}/${reponame}_pass.gpg 2>&1 |sed -n 's/^ .*<\\(.*\\)>\"$/\\L\\1/p'|sort|md5sum|grep -e '^${md5_keys}'", #lint:ignore:140chars
    ],
  }

  exec { "create key file ${title}":
    command  => "${configdir}/repo_${reponame}.sh exportkey | gpg --encrypt --always-trust ${keys} > ${git_home}/${::fqdn}/${reponame}_keyfile.gpg", #lint:ignore:140chars
    require  => [Exec["initialize borg repo ${reponame}", "create gpg private key for ${::fqdn}"], File["${git_home}/${::fqdn}"]],
    provider => 'shell',
    unless   => [
      # check if file contains key
      "A=`${configdir}/repo_${reponame}.sh exportkey|md5sum`; gpg --decrypt --output - ${git_home}/${::fqdn}/${reponame}_keyfile.gpg |md5sum|grep \$A", #lint:ignore:140chars
    ],
  }

  exec { "reencrypt key file ${title}":
    command => "gpg --decrypt ${git_home}/${::fqdn}/${reponame}_keyfile.gpg | gpg --encrypt --always-trust ${keys} > ${git_home}/${::fqdn}/${reponame}_keyfile.gpg", #lint:ignore:140chars
    require => [Exec["initialize borg repo ${reponame}", "create gpg private key for ${::fqdn}","create key file ${title}"], File["${git_home}/${::fqdn}"]], #lint:ignore:140chars
    unless  => [
      # check if file is encrypted with correct keys
      "gpg --decrypt -v --output /dev/null ${git_home}/${::fqdn}/${reponame}_pass.gpg 2>&1 |sed -n 's/^ .*<\\(.*\\)>\"$/\\L\\1/p'|sort|md5sum|grep -e '^${md5_keys}'", #lint:ignore:140chars
    ],
