...
class eit_haproxy::basic_config (
  ...
  $binds = functions::array_to_hash($web_bind_ports.map |$port| {
      $_ssl = if $port == 443 and $mode == 'http' {
        if $_use_native_acme {
          # Per-domain certs and their ACME bindings live in the crt-list,
          # generated by eit_haproxy::native_acme.
          'ssl crt-list /etc/haproxy/crt-list.txt alpn h2,http/1.1 strict-sni'
        } else {
          [
            'ssl',
            if $use_lets_encrypt {
