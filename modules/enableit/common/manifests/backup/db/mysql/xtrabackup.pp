...
class common::backup::db::mysql::xtrabackup (
  ...
  # To support multiple borg repo, we need to see if the dump_dir and the borg archive is same.
  ...
  # FIXME: also move more the rest of backup settings from role::db::mysql - so
  # role's don't have backup settings directly
