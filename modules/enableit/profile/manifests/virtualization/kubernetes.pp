...
class profile::virtualization::kubernetes (
  ...
  Optional[Sensitive[String]] $etcd_ca_key                = $::role::virtualization::kubernetes::etcd_ca_key,
  Optional[String]            $etcdclient_crt             = $::role::virtualization::kubernetes::etcdclient_crt,
  Optional[Sensitive[String]] $etcdclient_key             = $::role::virtualization::kubernetes::etcdclient_key,
  Optional[String]            $etcdserver_crt             = $::role::virtualization::kubernetes::etcdserver_crt,
  Optional[Sensitive[String]] $etcdserver_key             = $::role::virtualization::kubernetes::etcdserver_key,
  Optional[String]            $etcdpeer_crt               = $::role::virtualization::kubernetes::etcdpeer_crt,
  Optional[Sensitive[String]] $etcdpeer_key               = $::role::virtualization::kubernetes::etcdpeer_key,
  Optional[String]            $kubernetes_ca_crt          = $::role::virtualization::kubernetes::kubernetes_ca_crt,
  Optional[Sensitive[String]] $kubernetes_ca_key          = $::role::virtualization::kubernetes::kubernetes_ca_key,
  Sensitive[String]           $discovery_token_hash       = $::role::virtualization::kubernetes::discovery_token_hash,
  Optional[String]            $front_proxy_ca_crt         = $::role::virtualization::kubernetes::front_proxy_ca_crt,
  Optional[Sensitive[String]] $front_proxy_ca_key         = $::role::virtualization::kubernetes::front_proxy_ca_key,
  Optional[String]            $sa_pub                     = $::role::virtualization::kubernetes::sa_pub,
  Optional[Sensitive[String]] $sa_key                     = $::role::virtualization::kubernetes::sa_key,
  Sensitive[String]           $token                      = $::role::virtualization::kubernetes::token,
  ...
  class { 'kubernetes' :
    ...
    etcd_ca_key                   => unwrap($etcd_ca_key),
    etcdclient_crt                => $etcdclient_crt,
    etcdclient_key                => unwrap($etcdclient_key),
    etcdserver_crt                => $etcdserver_crt,
    etcdserver_key                => unwrap($etcdserver_key),
    etcdpeer_crt                  => $etcdpeer_crt,
    etcdpeer_key                  => unwrap($etcdpeer_key),
    kubernetes_ca_crt             => $kubernetes_ca_crt,
    kubernetes_ca_key             => unwrap($kubernetes_ca_key),
    discovery_token_hash          => unwrap($discovery_token_hash),
    kubernetes_front_proxy_ca_crt => $front_proxy_ca_crt,
    kubernetes_front_proxy_ca_key => unwrap($front_proxy_ca_key),
    sa_pub                        => $sa_pub,
    sa_key                        => unwrap($sa_key),
