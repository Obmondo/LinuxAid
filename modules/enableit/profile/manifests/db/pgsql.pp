...
class profile::db::pgsql (
  ...
  Optional[Eit_types::SimpleString]  $recovery_password    = $role::db::pgsql::recovery_password,
  ...
  Optional[Eit_types::SimpleString]  $replication_password = $role::db::pgsql::replication_password,
  ...
  if $mode == 'standby' {
    ...
    postgresql::server::recovery { 'hot_standby_recovery.conf':
      ...
      primary_conninfo => "host=${recovery_host} port=${recovery_port} user=${recovery_username} password=${recovery_password} application_name=${application_name}", #lint:ignore:140chars
    }
  }
