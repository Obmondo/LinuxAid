...
class saz_rsyslog::client (
  ...
  if $content_real {
    ...
    file { "${saz_rsyslog::params::rsyslog_d}client.conf":
      ensure => absent,
    }

    saz_rsyslog::snippet { '00_client_config':
      ensure  => present,
      content => template("${module_name}/client/config.conf.erb"),
    }

    saz_rsyslog::snippet { '50_client_remote':
      ensure  => $_remote_ensure,
      content => template("${module_name}/client/remote.conf.erb"),
    }

    saz_rsyslog::snippet { '99_client_local':
  ...
  if $imfiles {
    create_resources(saz_rsyslog::imfile, $imfiles)
  }
